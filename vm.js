	...
	if (testTxt) {
		...
		while (mem[ip] != 'exit')
			switch (mem[ip]) {
				...
				case 'jin': // jin arg1 (j++) - аналог cin (c++)
					in_num = readlineSync.question("input a number: ");
					mem[mem[ip + 1]] = parseInt(in_num);
					ip += 2
					break

				case 'jout': // jout arg1 (j++) - аналог cout (c++)
					console.log((mem[mem[ip + 1]]));
					ip += 2;
					break;

				case 'set': // set arg1 arg2 - поместить значение arg2 в arg1
					mem[mem[ip + 1]] = parseInt(mem[ip + 2]);
					ip += 3;
					break;

				case 'mov': // mov arg1 arg2 - скопировать значение arg2 в arg1
				...
				case 'sub': // sub arg1 arg2 arg3 - вычитание (arg3 - адрес суммы)
					mem[mem[ip + 3]] = mem[mem[ip + 1]] - mem[mem[ip + 2]];
					ip += 4;
					break;
				...
				case 'div': // div arg1 arg2 arg3 - целочисленное деление (arg3 - адрес деления)
					mem[mem[ip + 3]] = mem[mem[ip + 1]] / mem[mem[ip + 2]];
					ip += 4;
					break;
